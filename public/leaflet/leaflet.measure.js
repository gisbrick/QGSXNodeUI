(function (factory, window) {
    ...
    L.Control.Measure = L.Control.extend({
        ...
        _enableMeasureLine: function (ev) {
            //TOÑO quitamos este código, porque utilizaremos botones propios
    ...
    L.MeasureLable = L.Layer.extend({
        ...
        onAdd: function (map) {
            ...
            // Añadir listener para rotación del mapa
            if (map.on && typeof map.on === 'function') {
                map.on("rotate", this._updatePosition, this);
            }
            if (L.Browser.any3d) {
        ...
        //TOÑO modificamos la función para integrarla con el tipo de mediciones que queremos implementar
        addTo: function (map, mapUid) {
            map.addLayer(this);
            this.mapUid = mapUid;
            return this;
        },
        ...
        _updatePosition: function () {
            if (!this._map || !this.options.latlng) {
                return;
            }
            // Convertir latlng a layerPoint (coordenadas relativas al overlayPane rotado)
            var layerPoint = this._map.latLngToLayerPoint(this.options.latlng);
            // Convertir layerPoint a containerPoint (coordenadas relativas al contenedor, donde está el popupPane)
            var point = this._map.layerPointToContainerPoint(layerPoint);
            var is3D = L.Browser.any3d,
                offset = this.options.offset;
            if (is3D) {
                // Usar setPosition con transform para mapas rotados
                if (this._map._rotate && L.DomUtil.setPosition) {
                    L.DomUtil.setPosition(this._container, point);
                } else {
                    this._container.style.transform = 'translate3d(' + point.x + 'px, ' + point.y + 'px, 0)';
                }
            }
    ...
    L.MeasureAction = L.Handler.extend({
        ...
        _onMouseClick: function (event) {
            //TOÑO si el click lo hacemos sobre un boton del mapa, no hacemos nada
            if(window.mouseOverMapButton)return;
        ...
        _enableMeasure: function () {
             //TOÑO
            //deshabilitamos el uso de otros botones
            window.mouseOverButton = true
        ...
        _disableMeasure: function () {
             //TOÑO
            //habilitamos el uso de otros botones
                ...
                if (this._trail.points.length > 1) {
                    ...
                    if (this.options.model === "area") {
                        this._addLable(
                         ...
                         //TOÑO tras finalizar medición, desactivamos la herramienta
                         if(this._map.myUtils && this._map.myUtils.activeTool){
                            this._map.myUtils.activeTool(null);
                         }
                    } else {
                        this._addLable(
                            this._lastPoint,
        ...
        _numberFormat: function (number, decimals = 2) {
            ...
            return [
                ...
                //decimals ? "." +Math.abs(num - intPart).toFixed(decimals).slice(2)
                decimals ? "," +Math.abs(num - intPart).toFixed(decimals).slice(2)
                    : "",
            ].join("");
        },
